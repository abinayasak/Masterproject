...
def run_solver(make_gif, dimension):
    ...
    if dimension == "2D":
        ...
        v0 = Expression("10*x[0]", degree=1)
        v0 = Expression('x[0] <= 0.2 ? 0 : -85', degree=0)
    ...
    for i in range(N + 1):
        ...
        v_array[0][count] = v(0,10)
        v_array[1][count] = v(10,10)
        v_array[2][count] = v(20,10)
    ...
    plt.plot(t, v_array[0], label="(1,10)")
    plt.plot(t, v_array[1], label="(10,10)")
    plt.plot(t, v_array[2], label="(20,10)")
    plt.xlabel("t")
    plt.ylabel("v")
